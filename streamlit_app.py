...
def load_data():
    ...
    try:
        ...
        # --- 병합 ---
        if "ENCODED_MCT" not in data1.columns:
            st.error("⚠️ data1에 ENCODED_MCT 컬럼이 없습니다.")
            st.stop()
        if "ENCODED_MCT" not in data2.columns:
            st.error("⚠️ data2에 ENCODED_MCT 컬럼이 없습니다.")
            st.stop()

        merged_12 = pd.merge(data1, data2, on="ENCODED_MCT", how="inner")

        if not {"ENCODED_MCT", "TA_YM"}.issubset(data3.columns):
            st.error("⚠️ data3에 ENCODED_MCT 또는 TA_YM 컬럼이 없습니다.")
            st.stop()

        merged_all = pd.merge(merged_12, data3, on=["ENCODED_MCT", "TA_YM"], how="inner")

        st.success(f"✅ 병합 완료! (행 수: {len(merged_all):,}개)")
        return merged_all
